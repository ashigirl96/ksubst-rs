---
# Template generated by https://github.com/ashigirl96/repo-templates; do not edit downstream
---
 ...
 * Membership in the [Fedora CoreOS Crates Owners group](https://github.com/orgs/ashigirl96/teams/fedora-ashigirl96-crates-owners/members), which will give you upload access to crates.io
...
:warning:: if `origin` is not the name of the locally configured remote that points to the upstream git repository (i.e. `git@github.com:ashigirl96/ksubst-rs.git`), be sure to assign the correct remote name to the `UPSTREAM_REMOTE` variable.
...
- publish this release on GitHub:
  - [ ] find the new tag in the [GitHub tag list](https://github.com/ashigirl96/ksubst-rs/tags), click the triple dots menu, and create a release for it
...
- Fedora packaging:
  - [ ] update the `rust-ksubst` spec file in [Fedora](https://src.fedoraproject.org/rpms/rust-ksubst)
    - bump the `Version`
    - switch the `Release` back to `1%{?dist}`
    - remove any patches obsoleted by the new release
    - update changelog
  - [ ] run `spectool -g -S rust-ksubst.spec`
  - [ ] run `kinit your_fas_account@FEDORAPROJECT.ORG`
  - [ ] run `fedpkg new-sources $(spectool -S rust-ksubst.spec | sed 's:.*/::')`
  - [ ] PR the changes in [Fedora](https://src.fedoraproject.org/rpms/rust-ksubst)
